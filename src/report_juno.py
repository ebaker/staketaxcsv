...
junod query txs --home /tmp/.gaia --events message.sender=<wallet_address> \
      --node https://<RPCURL>:443 --limit 100

junod query txs --home /tmp/.gaia --events transfer.recipient=<wallet_address> \
      --node https://<RPCURL>:443 --limit 100
...
def main():
    ...
    if txid:
        ...
        report_util.run_exports(TICKER_JUNO, wallet_address, exporter, format)
...
def wallet_exists(wallet_address):
    line = "junod query account {} --home {} --node {} --output json".format(wallet_address, HOME, JUNO_NODE)
...
def txone(wallet_address, txid):
    line = "junod query tx {} --home {} --node={} ".format(txid, HOME, JUNO_NODE)
...
def _query_blockchain(wallet_address, limit, page, chain_id, sender=True):
    ...
    line = "junod query txs --events '{}' ".format(events)
    line += "--limit {} --node '{}' --page {} --chain-id '{}' --home {} ".format(limit, JUNO_NODE, page, chain_id, HOME)
